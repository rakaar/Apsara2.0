...
const URL = 'https://cms.iit-techambit.in/api/node/article?fields[node--article]=uid,title,created,body,field_image&include=field_image,field_image.image,field_image.image.file--file&fields[field_image]=image&fields[file--file]=uri,url&include=uid'
const headers = {
  headers: {
  ...
  useEffect(() => {
    axios
      ...
      .then(res => {
        ...
        articleCards = data.map(item => {
          return {
            image: '', // check this later
